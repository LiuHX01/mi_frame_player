...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const calcPercent = () => {
            ...
            // console.log(state.frame, props.timeRange.length, state.percent)
        }
        ...
        const clickFrame = ($event) => {
            ...
            if (target <= 1) {
                target = 0
            } else if (target >= props.timeRange.length - 1) {
                target = props.timeRange.length - 1
            }

            console.log('clickFrame', target)
            // console.log(target)
            jumpFrame(target)
        }
    ...
    <div>
        <div class="control-panel">
            ...
            <div ref="sliderRef" class="my-slider-control" @click="clickFrame"
                ...
                <div class="now-frame"
                    :style="{ 'display': 'inline-block', 'left': calcNowFrame, height: '20px', width: '2px', 'color': 'yellow', 'background-color': 'green' }"></div>
            </div>
            <!-- </el-main> -->
            <!-- </el-container> -->
        </div>
